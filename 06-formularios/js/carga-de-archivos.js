...
window.addEventListener("load", function(event) {
  var inputsFile = document.querySelectorAll('.input-carga-de-archivo');
  methodAssignFileInput("change", selectingFiles, inputsFile);

  var buttonsFile = document.querySelectorAll('.button-loader-carga-de-archivo');
...
function windowSize() {
  let containerInputsFile = document.querySelectorAll('.container-carga-de-archivo');
  for(let e of containerInputsFile) {
    if (e.offsetWidth < 340) {
      e.classList.add('responsive','small-fileupload');
    } else if (e.offsetWidth < 652) {
      e.classList.add('responsive');
      e.classList.remove('small-fileupload');
    } else {
      e.classList.remove('responsive', 'small-fileupload');
    }
  }
}
...
function selectingFiles()  {
  ...
  const button = parentCarga.querySelector('.button-loader-carga-de-archivo');
  let fileName = '';
...
function assignName(container, fileName) {
  const containerNameFile = container.querySelector('.file-name-carga-de-archivo');
...
async function clickButtonFile() {
  ...
  const inputFile = parent.querySelector('.input-carga-de-archivo');
  const idInput = inputFile.id;
  const containerLoader = parent.querySelector('.load-carga-de-archivo');
  ...
  if (inputFile.value) {
    ...
    const container = containerParent.querySelector('.attached-files-carga-de-archivo');
...
function errorInputFile(input, active, message) {
  ...
  const containerAlert = containerParent.querySelector('.alert-carga-de-archivo');
...
function createElement(container, name, type) {
  ...
  newDivAttached.classList.add('attached-file-carga-de-archivo');
  newDivAttached.setAttribute("tabindex", "0");

  const newDivContainerIcon = document.createElement('div');
  newDivContainerIcon.classList.add('icon-text-carga-de-archivo');

  const newDivIconFile = document.createElement('div');
  newDivIconFile.classList.add('file-alt-carga-de-archivo');

  const newDiv = document.createElement('div');
  newDiv.classList.add('container-text-name-carga-de-archivo');
  const span1 = document.createElement('span');
  span1.classList.add('text-name-carga-de-archivo');
  ...
  newButtonIconTrash.classList.add('trash-alt-1-carga-de-archivo');
...
async function removeAttachment() {
  ...
  const inputFile = containerAll.querySelector('.input-carga-de-archivo');
  const idInput = inputFile.id;
  const containerLoader = containerAll.querySelector('.load-carga-de-archivo');
  const containerName = container.querySelector('.text-name-carga-de-archivo');
